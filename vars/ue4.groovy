...
def runProjectTests(Map config = [:]) {
    lock(resource: "UnrealBuildTool-${NODE_NAME}") {
        def editorCMD = getEditorCMDPath(config)
        platform.executeScript("\"${editorCMD}\" \"${config.projectPath}\" -game -ExecCmds=\"Automation RunTests Project;Quit\" -unattended -NoSound -nopause -nosplash -NullRHI  -testexit=\"Automation Test Queue Empty\" -log -ReportOutputPath=\"${config.reportPath}\"", 'Run Project Automation Tests')
    }
}
