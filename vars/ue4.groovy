...
def runProjectTests(Map config = [:]) {
    lock(resource: "UnrealBuildTool-${NODE_NAME}") {
        def editorCMD = getEditorCMDPath(config)
        platform.executeScript("\"${editorCMD}\" \"${config.projectPath}\" -game -ExecCmds=\"Automation RunTests ${config.tests};Quit\" -unattended -NoSound -nopause -nosplash -NullRHI -testexit=\"Automation Test Queue Empty\" -log -stdout -fullstdoutlogoutput -ReportOutputPath=\"${config.reportPath}\"", 'Run Project Automation Tests')
    }
}
